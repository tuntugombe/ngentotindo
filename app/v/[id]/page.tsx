...
export default async function Video({ params }: PageProps) {
    ...
    const file = data.result[0];
        ...
        publisher: {
            '@type': 'Organization',
            name: `${SITENAME}`,
            logo: 'https://ngentotindo.pages.dev/favicon.ico'},
            author: {
                '@type': 'Person',
                name: 'admin',
                url: 'https://ngentotindo.pages.dev'
              }
        }
        const jsonLd2 = {
        '@context': 'https://schema.org',
        ...
        interactionStatistic: {
            ...
            interactionType: {
                '@type': `ReadAction`,
                target: `https://ngentotindo.pages.dev/v/${file.filecode}`
            }  
        }
        }
    return (
